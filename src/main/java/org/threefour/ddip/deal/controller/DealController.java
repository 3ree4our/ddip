...
public class DealController {
  ...
  public ResponseEntity<String> checkWaitingStatus(@PathVariable Long productId, @RequestHeader("Authorization") String authorizationHeader) {
    String accessToken = authorizationHeader.substring(7).trim();
    if (!FormatValidator.hasValue(accessToken)) {
      throw new TokenNoValueException(TOKEN_NO_VALUE_EXCEPTION_MESSAGE);
    }
    ...
    DealStatus dealStatus = dealService.checkWaitingStatus(productId, buyerId);
    String status = dealStatus.name();
